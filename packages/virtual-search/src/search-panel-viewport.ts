...
export function attachSearchPanelViewport(
  panel: HTMLElement,
  {
    anchor = "preserve",
    enabled = true,
  ...
  const sync = () => {
    ...
    if (view.performance.now() < settleUntil) {
  ...
  const scheduleSettledSync = () => {
    settleUntil = view.performance.now() + VIEWPORT_SETTLE_MS;
    scheduleSync();
  };
  ...
  view.addEventListener("scroll", scheduleSettledSync, { passive: true });
  ...
  return () => {
    ...
    view.removeEventListener("scroll", scheduleSettledSync);
